...
export async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    if (args.command === 'approve') return print(await approveModelPlanFile({ inputPath: args.input, outputPath: args.output, conflictPath: args.conflicts, reviewer: args.reviewer, reason: args.reason, allowAmbiguous: args.allowAmbiguous === true }), wantsJson);
...
async function cleanup(manifest, adapter, args) {
  ...
  observer.record('cleanup', { phase: 'started', dryRun });
  const registry = await ResourceRegistry.restore(artifactDir, runId, `${manifest.platform.test_data_prefix}${runId}-`);
  registry.observer = observer;
  const result = await registry.cleanup((resource) => adapter.deleteResource(resource, { baseUrl, runId, manifest, observer }), { dryRun });
  observer.record('cleanup', { phase: 'finished', ok: result.ok, pending: result.pending });
  const file = await observer.persist();
  return { ...result, command: 'cleanup', runId, dryRun, observations: { ...observer.summary(), file: file.slice(artifactDir.length + 1).replaceAll('\\', '/') } };
}
