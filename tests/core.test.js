...
import { resolveRunProvenance } from '../src/provenance.js';
...
test('adapter provenance includes explicitly declared dependency files', async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-soak-provenance-'));
  try {
    await fs.writeFile(path.join(dir, 'adapter.js'), 'export {}');
    await fs.writeFile(path.join(dir, 'helper.js'), 'export const value = 1');
    const value = { ...manifest, adapter: './adapter.js', adapter_files: ['./helper.js'] };
    const first = await resolveRunProvenance({ manifestPath: path.join(dir, 'platform.manifest.json'), manifest: value });
    await fs.writeFile(path.join(dir, 'helper.js'), 'export const value = 2');
    const second = await resolveRunProvenance({ manifestPath: path.join(dir, 'platform.manifest.json'), manifest: value });
    assert.notEqual(first.adapter_fingerprint, second.adapter_fingerprint);
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
  }
});
