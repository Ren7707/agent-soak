...
- 源码操作证据提取，以及字段、实体、路由和操作之间的可审计关联
- 候选契约的字段级证据摘要传递和元数据冲突审查
- 带审核人、理由和时间记录的测试计划审批流程
- 测试计划质量评分、证据覆盖和风险覆盖矩阵
- 基于源码操作证据检查创建、查询、更新、删除等业务操作覆盖
- 状态变更场景强制要求权威结果观察，避免把请求成功或按钮完成误判为业务成功
...
`scenarios`、`diagnostics`、`cleanup`、`preflight` 和 `observations` 字段，便于 CI 与 Skill
...
编排成创建、查询、更新和删除等真实场景；它不会直接生成平台专用 Adapter。字段候选
会在实体明确或源码邻近关系足够可靠时携带 `entity`、`operations`、`routes` 和
`operation_refs`，这些关联只作为可追溯证据，不会把扫描器猜测直接升级为产品规则。
候选契约中还会保留
...
如果契约携带了明确的实体操作证据，计划还必须为每个实体操作提供对应场景；例如源码
同时存在 `POST /devices` 和 `GET /devices/:id` 时，只覆盖注册而没有详情查询场景会产生
`required_operation_uncovered`，避免“字段测到了但业务闭环没测到”。
`create`、`update`、`delete` 和 `transition` 场景还必须声明 `observe` 步骤或
`observation` transport，用于读取权威详情、列表、状态或删除结果；只有接口返回成功而
没有业务状态观察的计划会产生 `observation_missing`。
